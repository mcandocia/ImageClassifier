...
class neural_network(object):
	...
	self.cat_labels = self.fetcher.valid_names
        ...
        self.learning_rate = theano.shared(np.float32(
                base_learning_rate * (1-momentum)))
        self.index = index = T.lscalar()
    ...
    def reduce_learning_rate(self):
        ...
        new_rate = np.float32(
            current_rate * (1 - (1 - self.learning_decay_per_epoch)/\
                                float(self.n_train_batches)))
        ...
        try:
            while (self.epoch < self.n_epochs) and not self.done_looping:
                ...
                for train_i in range(self.validation_frequency):
                    ...
                    if train_i % 50 == 0:
                        print str(train_i) + ' | ' + \
                            str(train_i + (self.epoch) * self.validation_frequency)
                ...
                self.epoch += 1
                print "epoch ended; learning rate: %s" % round(self.learning_rate_raw,5)
    ...
    def test_data(self,x,y):
        pass
    def save_network(self,mode=None):
        nd = dict()
        paramdict = dict()
        extraparams = []
...
def load_network_isolate(filename,modified_batch_size=None):
    #load pickle
